<%@page import="cn.train.entity.ShowOrder"%>
...
.showorder_top {
	width: 400px;
	height: 20px;
	background-color: #41BAE2;
}

.info_right_bottom_in table, .info_right_bottom_in table tr td {
	border: 1px solid #71C5E3;
}
...
.showorder_bottom {
	width: 398px;
	height: 40px;
	border-right: 1px solid #71C5E3;
	border-bottom: 1px solid #71C5E3;
	border-left: 1px solid #71C5E3;
}
...
<body>
	...
	<%
        ...
        List<ShowOrder> list=(List<ShowOrder>)request.getAttribute("list");
      %>
	<div class="main">
		<!-- 头部开始 -->
		<div class="main_top">
		...
		<div class="info">
			...
			<div class="info_right">
				...
				<div class="info_right_bottom">
					...
					ShowOrder showOrder=list.get(i);
					%>
					<div class="info_right_bottom_in">
						<div class="showorder_top"></div>
						<div class="showorder_middle">
							<table>
								<tr height="40px">
									<td width="50px">序号</td>
									<td width="150px">车次信息</td>
									<td width="100px">旅客信息</td>
									<td width="100px">票款余额</td>
								</tr>
								<tr height="100px">
									<td><%=i+1 %></td>
									<td><%=showOrder.getOrderStartDate()%>&nbsp;&nbsp;<%=showOrder.getStartDate()%>开<br>
										<%=showOrder.getTrainNo()%>&nbsp;&nbsp;<%=showOrder.getStart()%>--<%=showOrder.getArrive()%>
									</td>
									<td><%=showOrder.getName()%></td>
									<td><%=showOrder.getPrice() %></td>
								</tr>
							</table>
						</div>
						<div class="showorder_bottom">
							<a
								href="delOrder?username=<%=(String)session.getAttribute("username") %>&orderStartDate=<%=showOrder.getOrderStartDate()%>&trainNo=<%=showOrder.getTrainNo()%>">
								<button class="showorder_tui">退票</button>
							</a> <a href="delOrder?username=<%=(String)session.getAttribute("username") %>&orderStartDate=<%=showOrder.getOrderStartDate()%>&trainNo=<%=showOrder.getTrainNo()%>"><button class="showorder_gai">改签</button></a>
						</div>
					</div>
					<%} %>
				</div>
